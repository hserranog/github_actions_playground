...
class VistaCrearIngReceta(QDialog):
    ...
    def __init__(self, ingredienteReceta, interfaz, ingredientes, ingredientesReceta):
        """
        ...
        self.todos_ingredientes = ingredientes
        self.ingredientesReceta = ingredientesReceta
        ...
        if (ingredienteReceta == None):
            ...
            nombres_en_receta = {i["ingrediente"] for i in self.ingredientesReceta}
            # Filtrar los ingredientes que no están en la receta
            self.ingredientes[:] = [
                ingrediente
                for ingrediente in self.ingredientes
                if ingrediente["nombre"] not in nombres_en_receta
            ]
        else:
            titulo = "Editar ingrediente de receta"
            nombres_en_ingredientes = {i["nombre"] for i in self.ingredientes}
            for i in self.ingredientesReceta:
                if i["ingrediente"] not in nombres_en_ingredientes:
                    self.ingredientes.append({
                        "nombre": i["ingrediente"],
                        "unidad": i.get("unidad", ""),
                        "valor": i.get("valor", 0),
                        "sitioCompra": i.get("sitioCompra", ""),
                    })
        ...
        if (ingredienteReceta != None):
            ...
            self.combobox_ingredientes.setEnabled(False)
